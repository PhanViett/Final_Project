...
class NoiDungThucHanhById(Resource):
    ...
    def put(self, id):
        ...
        noi_dung_thuc_hanh = schema.load(request.json, instance= noi_dung_thuc_hanh)
        ten = request.json.get('ten')
        if ten is not None:
            noi_dung_thuc_hanh.ten_khong_dau = clean_string(ten)
        db.session.commit()
        return {"msg": "sửa thành công!",
                "results":schema.dump(noi_dung_thuc_hanh)}, HttpCode.OK,
