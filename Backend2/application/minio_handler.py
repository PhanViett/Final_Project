...
class MinioHandler:
    ...
    def presigned_get_object(self, bucket_name, object_name, expire_time=timedelta(days=7)):
        # Request URL expired after 7 days
        url = self.client.presigned_get_object(
            bucket_name=bucket_name, object_name=object_name, expires=expire_time
        )
        return url
    ...
    def save_duocsi_files(self, bucket, file_data, file_name):
        try:
            ...
            self.client.put_object(
                bucket_name=bucket,
                object_name="duoc_si/" + object_name,  # Path + Name.ext
                data=file_data,
                length=-1,
                part_size=10 * 1024 * 1024,
            )

            return bucket + "/duoc_si" + "/" + object_name
